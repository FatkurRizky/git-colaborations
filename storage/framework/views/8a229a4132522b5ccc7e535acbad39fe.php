    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div class="flex items-center gap-2">
                <a href="<?php echo e(route('rekon.export.pdf')); ?>"
                ...
                <a href="<?php echo e(route('rekon.export.excel')); ?>"
                    class="inline-flex items-center px-4 py-2 bg-white border border-slate-300 text-slate-700 text-sm font-bold rounded-xl hover:bg-slate-50 transition shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
                ...
                <h3 class="text-2xl font-black text-slate-900 mt-1 font-mono text-emerald-600">
                    Rp<?php echo e(number_format($totalKasHariIni, 0, ',', '.')); ?></h3>
            </div>
            <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
                <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">Status Terakhir</p>
                <div class="mt-2">
                    <span
                        class="px-3 py-1 bg-green-100 text-green-700 text-xs font-black rounded-lg uppercase"><?php echo e(strtoupper($statusTerakhir)); ?></span>
                </div>
            </div>
            <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
                <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">Total Selisih (Bulan Ini)</p>
                <h3
                    class="text-2xl font-black mt-1 font-mono <?php echo e($totalSelisihBulanIni < 0 ? 'text-red-600' : 'text-green-600'); ?>">
                    <?php if($totalSelisihBulanIni < 0): ?>
                        -Rp <?php echo e(number_format(abs($totalSelisihBulanIni), 0, ',', '.')); ?>

                    <?php else: ?>
                        Rp <?php echo e(number_format($totalSelisihBulanIni, 0, ',', '.')); ?>
